...
数据库迁移统一由 `backend/db.py` 管理，核心原则如下：

1. **必须版本化**：所有 Schema 变更必须登记到迁移版本（`schema_migrations.version`），禁止仅依赖手工改表。
2. **支持跨版本升级**：迁移按版本顺序执行，旧版本数据库可直接升级到最新版本，不要求逐个中间版本手动升级。
3. **启动自动迁移**：应用启动时通过 `init_db()` 自动执行未完成迁移。
4. **迁移需幂等**：迁移步骤应可重复执行，不应因重复执行而失败（例如：新增列前先检查是否存在）。
5. **包含数据回填**：新增字段若影响已有数据语义，必须在迁移中完成必要回填/推断（而不是留给人工处理）。
6. **索引与 FTS 同步维护**：涉及查询能力的结构（索引、FTS、触发器）必须纳入迁移，并保证升级后可用。

当前实现要点：

- 迁移版本表：`schema_migrations`（固定 `id=1`，记录当前版本）
- 迁移入口：`migrate_schema(db)`
- 启动执行：`init_db()` 中调用迁移
- 最新版本常量：`LATEST_SCHEMA_VERSION`

提交流程要求：

1. 新增迁移步骤并更新 `LATEST_SCHEMA_VERSION`
2. 编写/更新测试，至少覆盖：
   - 旧版本数据库升级到最新版本
   - 关键字段或关键查询能力（如 FTS）在升级后可用
3. 在项目虚拟环境中执行后端测试并确保通过
